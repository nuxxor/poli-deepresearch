...
async function runMarketResearch(
  ...
  const runClaims = shouldRunClaimExtraction(market, combination.finalMode, startedAt, extracted.docs);
  const localClaims = runClaims
    ? await extractClaimsWithLocalModel(market, extracted.docs).catch(() => null)
    : null;
  ...
  const adversarialResult = await runAdversarialReview({
    ...
    evidence: extracted.docs,
    claims: provisionalEvidenceArtifacts.claims,
    crossMarketContext: crossMarketContext ?? undefined,
  ...
  const probabilisticForecast = buildProbabilisticForecast({
    ...
    claims: evidenceArtifacts.claims,
    evidence: extracted.docs
  });
  ...
  const runOfflineSummaryFlag = shouldRunOfflineSummary(
    ...
    startedAt,
    evidenceArtifacts.claims.length,
    citations.length
  );
  const offlineSummary = runOfflineSummaryFlag
    ? await buildOfflineSummary(market, finalOpinion, evidenceArtifacts.claims, extracted.docs, citations).catch(
        () => null
      )
    : null;
...
async function buildPlannerForMarket(market: MarketContext): Promise<LocalPlanner> {
...
function pickExtractionCandidates(
...
function shouldRunClaimExtraction(
  market: MarketContext,
  finalMode: ResearchFinalMode,
  startedAt: number,
  docs: ProviderSearchResultItem[] | { length: number }
): boolean {
  if (docs.length === 0) {
    return false;
  }

  if (finalMode === "failed" || finalMode === "local_floor" || finalMode === "direct_only") {
    return false;
  }
  ...
  const { category } = market.canonicalMarket;
  return category !== "world" && category !== "politics";
}

function shouldRunOfflineSummary(
  market: MarketContext,
  finalMode: ResearchFinalMode,
  startedAt: number,
  claimCount: number,
...
async function awaitSignalsWithBudget(
...
function buildEmptySignalsSummary(
  market: MarketContext,
  queryPlan: SearchQueryPlan,
  error: unknown
) {
